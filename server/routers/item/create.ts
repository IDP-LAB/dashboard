  ...
  handler: async ({ response, request }) => {
    ...
    try {
      ...
      if (schema.category.id) {
        ...
        if (!category) return response.code(404).send({ message: `Categoria com ID ${schema.category.id} não encontrada.` })
      ...
      if (schema.tags) {
        for (const tagInput of schema.tags) {
          ...
          if (tagInput.id) {
            ...
            if (!tagEntity) return response.code(404).send({ message: `Tag com ID ${tagInput.id} não encontrada.` })
      ...
      if (fields.groupUuid) {
        ...
        if (!group) return response.code(404).send({ message: `Group ${fields.groupUuid} não encontrado.` })
      } else {
