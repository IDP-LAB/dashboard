...
export default new Router({
  ...
  path: '/group/:groupUuid/files/:fileId/download',
  ...
  methods: {
    async get({ reply, request }) {
      ...
      try {
        // Buscar o arquivo
        const file = await File.findOne({ where: { id: parseInt(fileId) }, relations: { group: true } })
        ...
        try {
          ...
          reply.header('Content-Disposition', `attachment; filename="${file.originalName || file.filename}"`)
          reply.header('Content-Type', file.mimeType)
          reply.header('Content-Length', file.size.toString())
          
          // Pipe do stream diretamente para a response
          return reply.type(file.mimeType).send(fileStream)
        } catch (error) {
